...
> **Amendment 2026-08-11 — source consolidation, chưa deploy:** GHN connector 0.2.0 dùng một application lifecycle chung cho Woo admin và Toolkit adapter tùy chọn; canonical order meta là `_openship_ghn_*` với legacy read fallback cô lập. Prototype VietQR custom đã bị gỡ; address và VietQR đều **REUSE-FIRST / chưa chọn nguồn**. Production vẫn ở GHN 0.1.1 + Toolkit 1.1.2; payment/shipping/runtime chưa đổi. Toolkit Composer package được giữ cho address fields/data và shipping rules tới controlled cutover. Xem `docs/VIETNAM-TOOLKIT-DECOUPLING.md`.

...
const assetFiles = [
  ...[...imageMetadata.keys()].map((sourcePath) => path.join(publicRoot, sourcePath.replace(/^\//, ''))),
  ...rootAssets.map((relative) => path.join(publicRoot, relative)),
].filter((absolute, index, files) => files.indexOf(absolute) === index);
...
if (content.blogPosts.length !== 5 || content.assets.length === 0) {
  throw new Error(`Unexpected transfer counts: blog=${content.blogPosts.length}, assets=${content.assets.length}`);
}

fs.mkdirSync(path.dirname(outputFile), { recursive: true });
fs.writeFileSync(outputFile, `${JSON.stringify(content, null, 2)}\n`, 'utf8');
console.log(`Extracted editorial content: blog=${content.blogPosts.length} assets=${content.assets.length}`);

...
| Validation boundary | Retry 2026-08-11: saved Token PASS direct Province API (`200`, 65 tỉnh); saved ShopId khớp store duy nhất của staging account. Preview xác thực được nhưng trả `400 Address convert from fail` vì store đó không có cả pickup address legacy lẫn v2 (`address`/ward/district và `address_v2`/`ward_id_v2` đều trống). Connector đã trả về disabled/Test; order `145` và draft product `144` chỉ dùng test rồi xóa; không tạo GHN shipment. Owner phải cấu hình pickup address trong 5Sao trước khi chạy lại lifecycle |

...
| `lyli-ghn-connector` | 0.1.1 | `web/app/plugins/lyli-ghn-connector/` trong repo này | Nội bộ | **Đã deploy/active; connector tắt ở Test sau E2E PASS**; provider GHN thủ công cho framework shipment của Toolkit, không có live checkout rate | Developer bảo trì code; Shop Owner nhập credential/chính sách và chỉ bật Production sau controlled cutover | Option cấu hình + Token không autoload; metadata shipment chuẩn của Toolkit chỉ sinh sau thao tác create; print token không persist và trang in mở trực tiếp trên exact GHN allowlist | DB dump + immutable release | Tắt connector trước, rồi deactivate; rollback `20260810210244` + DB nếu đã tạo shipment | Thấp–trung bình — Test Preview/Create/idempotency/Detail/Sync/Print/Cancel PASS; không webhook; không auto-create; không gọi GHN ở checkout |

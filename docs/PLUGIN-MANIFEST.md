...
| Vietnam Store Toolkit for WooCommerce | 1.1.2 historical | WPackagist `wpackagist-plugin/yoohw-vietnam-store-tools`; WordPress.org stable 1.1.2 | GPLv2+ | **REMOVED / INACTIVE** từ Release B `20260811151035`; không còn trong Composer/artifact | Không còn runtime owner | Historical options/order meta được giữ trong DB; GHN legacy `_vck_shipping_*` chỉ còn read-only compatibility | DB backup `20260811144446` + rollback Release A | Rollback về `20260811145842` nếu cần phục hồi package; không uninstall/xóa historical data | Không còn runtime risk; giữ dòng này để truy vết lịch sử |
| `lyli-ghn-connector` | 0.2.1 | `web/app/plugins/lyli-ghn-connector/` trong repo này | Nội bộ | **DEPLOYED / ACTIVE CODE / DISABLED TEST**; standalone Woo lifecycle, không live checkout rate | Developer bảo trì code; Shop Owner nhập credential/chính sách và chỉ bật Production sau controlled cutover | Private non-autoload Token option; canonical `_openship_ghn_*`; read fallback `_lyli_ghn_*` rồi read-only `_vck_shipping_*`; không persist Token/print token | DB dump + immutable release | Disable plugin/connector; rollback Release A | Trung bình — full GHN Test lifecycle PASS; Production gateway chưa được bật |
| `lyli-vietnam-address` | 0.1.0 | Repo adapter + pinned `thanglequoc/vietnamese-provinces-database` `v4.0.0` JSON | Adapter nội bộ; data MIT | **DEPLOYED / ACTIVE**; sole Vietnam address owner cho Classic Checkout/Account | Developer pin/update data; owner edits addresses through Woo UI | Woo customer/order `state` province code + `city` ward code; không option/plugin table | DB dump + immutable release | Disable plugin leaves Woo address values intact; rollback Release A | Thấp–trung bình — checksum 34/3.321 PASS; Classic Checkout/address compatibility PASS; Blocks deferred |
| `lyli-vietqr-bacs` | 0.1.0 | Repo adapter + Composer `liopay/vietqr:1.0.0`, `chillerlan/php-qrcode:6.0.1` | Adapter nội bộ; libraries MIT / MIT+Apache-2.0 | **DEPLOYED / ACTIVE CODE / INTEGRATION DISABLED**; native BACS presentation only | Developer pins code; owner enters merchant bank values and enables later | Woo Integration settings only; no webhook/payment secret/order-status writes | DB dump + immutable release | Disable adapter; native BACS/order data remains | Thấp–trung bình — local deterministic QR; no SaaS; owner chưa bật |
| `lyli-shipping-policy` | 0.1.0 | `web/app/plugins/lyli-shipping-policy/` trong repo | Nội bộ | **DEPLOYED / ACTIVE**; native Woo sở hữu zone/method/fee; guard chỉ loại rate `flat_rate` tên `Vận chuyển` khi `contents_cost > 100.000.000` VND hoặc package weight `> 1 kg` | Developer; không có owner settings UI | Không tạo option/table/order meta; chỉ dùng package/rate hiện tại | Không cần dữ liệu riêng; immutable release cho code | Deactivate để bỏ hai eligibility ceiling | Thấp — không shipping engine, SQL/AJAX/REST; 23 test + runtime matrix 8/8 PASS |
...
|---|---|---|---|---|---|
| SePay Gateway | 1.1.23 | Riêng của SePay (sepay.vn) — không trên WPackagist | Thương mại | **DEFERRED / OPTIONAL** | Chỉ xem xét nếu sau này cần automatic bank reconciliation/webhook; không triển khai song song với `lyli-vietqr-bacs`. Giữ nghiên cứu lịch sử, không thuộc V1 transfer UI trước mắt |

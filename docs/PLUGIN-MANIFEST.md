...
| `lyli-ghn-connector` | 0.2.1 source / 0.1.1 runtime | `web/app/plugins/lyli-ghn-connector/` trong repo này | Nội bộ | **0.2.1 NOT DEPLOYED**; một application lifecycle dùng chung cho Woo admin và optional address/Toolkit adapters; không có live checkout rate | Developer bảo trì code; Shop Owner nhập credential/chính sách và chỉ bật Production sau controlled cutover | Private non-autoload Token option; canonical `_openship_ghn_*`; read fallback `_lyli_ghn_*` rồi `_vck_shipping_*`; không persist Token/print token | DB dump + immutable release | Runtime hiện vẫn 0.1.1; cutover riêng và rollback release trước đó | Trung bình — source network-free/static PASS; 0.2.1 chưa runtime cutover |
| `lyli-vietnam-address` | 0.1.0 source only | Repo adapter + pinned `thanglequoc/vietnamese-provinces-database` `v4.0.0` JSON | Adapter nội bộ; data MIT | **NOT DEPLOYED / NOT ACTIVE**; replacement path cho Toolkit address | Developer pin/update data; owner edits addresses through Woo UI | Woo customer/order `state` province code + `city` ward code; no plugin option | DB dump + immutable release | Activate only in controlled Toolkit cutover; remove plugin leaves Woo address values intact | Thấp–trung bình — static checksum data, Classic Checkout covered; Blocks gate remains |
| `lyli-vietqr-bacs` | 0.1.0 source only | Repo adapter + Composer `liopay/vietqr:1.0.0`, `chillerlan/php-qrcode:6.0.1` | Adapter nội bộ; libraries MIT / MIT+Apache-2.0 | **NOT DEPLOYED / NOT ACTIVE**; native BACS presentation only | Developer pins code; owner enters merchant bank values and enables later | Woo Integration settings only; no webhook/payment secret/order-status writes | DB dump + immutable release | Disable adapter; native BACS/order data remains | Thấp–trung bình — local deterministic QR; no SaaS; low-adoption payload library mitigated by pin/source audit/tests |
| `lyli-shipping-policy` | 0.1.0 source | `web/app/plugins/lyli-shipping-policy/` trong repo | Nội bộ | Native Woo vẫn sở hữu zone/method/fee; plugin nhỏ chỉ loại rate `flat_rate` tên `Vận chuyển` khi `contents_cost > 100.000.000` VND hoặc package weight `> 1 kg` | Developer; không có owner settings UI | Không tạo option/table/order meta; chỉ dùng package/rate hiện tại | DB dump không cần dữ liệu riêng; immutable release cho code | Deactivate để bỏ hai eligibility ceiling | Thấp — không tạo shipping method, không SQL/AJAX/REST; tests network-free khóa boundary và rate scope |

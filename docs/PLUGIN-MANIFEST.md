...
| `lyli-ghn-connector` | 0.2.0 source / 0.1.1 runtime | `web/app/plugins/lyli-ghn-connector/` trong repo này | Nội bộ | **0.2.0 NOT DEPLOYED**; một application lifecycle dùng chung cho Woo admin và optional Toolkit adapter; không có live checkout rate | Developer bảo trì code; Shop Owner nhập credential/chính sách và chỉ bật Production sau controlled cutover | Private non-autoload Token option; canonical `_openship_ghn_*`; read fallback `_lyli_ghn_*` rồi `_vck_shipping_*`; không persist Token/print token | DB dump + immutable release | Runtime hiện vẫn 0.1.1; cutover riêng và rollback release trước đó | Trung bình — source network-free/static PASS; 0.2.0 chưa runtime cutover |

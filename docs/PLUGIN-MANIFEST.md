...
| `lyli-ghn-connector` | 0.2.0 source / 0.1.1 runtime | `web/app/plugins/lyli-ghn-connector/` trong repo này | Nội bộ | **0.2.0 NOT DEPLOYED**; GHN core dùng WooCommerce CRUD/admin, Toolkit chỉ là optional adapter; không có live checkout rate | Developer bảo trì code; Shop Owner nhập credential/chính sách và chỉ bật Production sau controlled cutover | Private Token option; canonical `_lyli_ghn_*` order meta; legacy `_vck_shipping_*` read fallback; print token không persist | DB dump + immutable release | Runtime hiện vẫn 0.1.1; cutover riêng và rollback release trước đó | Trung bình — source unit/static PASS, nhưng 0.2.0 chưa runtime/E2E |
| VietQR BACS for WooCommerce | 0.1.0 | `web/app/plugins/vietqr-bacs-for-woocommerce/` trong repo này | GPL-2.0-or-later | **SOURCE ONLY / NOT DEPLOYED / DISABLED BY DEFAULT**; bổ sung QR cho native BACS, không gateway mới | Developer deploy; Shop Owner nhập bank data và enable sau privacy gate | `woocommerce_vietqr_bacs_settings`; không có merchant data trong source; QR URL chỉ render trên order BACS phù hợp | DB dump + immutable release | Deactivate plugin; native BACS tiếp tục độc lập | Trung bình — network-free tests PASS; cần cutover/runtime regression riêng; không autobank/webhook/auto-paid |
